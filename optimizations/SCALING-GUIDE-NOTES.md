...
| Boot disk | **pd-ssd or hyperdisk-balanced, ≥200 GB** — pd-balanced <100 GB caps at 3,000 IOPS → gVisor shim freezes/CRI timeouts at 2,000-pod bursts (pp. 4-5, 8-9, 14) | **unset** — `kops create cluster` in `run:262-283` passes no `--node-volume-size`/type, so nodes get kops GCE defaults | **Δ — likely our cheapest untested knob for the refill ceiling.** The measured 70-85/s per-pool refill wall (ROUND7 item 8a) is attributed to node-local I/O; PDF says exactly this failure mode (CRI `context deadline exceeded`, p. 14) starts at boot-disk IOPS. Set explicitly before buying the NVMe legs. |
...
| Scheduler | "High-Throughput Scheduler" named, zero numbers (p. 3) | `kubeAPIQPS=800/1600` (`run:256-257`) | ✓ ahead |
