...
## Review round 2 (2026-07-23, Copilot — 1 finding, valid, fixed in `1905157`)

Critical FlowSchema wildcard verbs (claims rule + leases rule) narrowed to
hot-path writes: claims/sandboxes/pods `update,patch`; leases
`get,create,update`. list/watch/relists/reads/deletes fall to bulk by
design (documented in manifest + class table). Reply notes the PR-body
routing counts were measured with wildcard verbs (small fraction of
critical's 51.0k shifts to bulk; neutrality/zero-rejection unaffected).
Thread resolved; reply discussion_r3639979383.
