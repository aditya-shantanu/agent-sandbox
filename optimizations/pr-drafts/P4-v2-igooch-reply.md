# PR #1256 — reply to igooch's review (draft)

Placeholders (`{{PLACEHOLDER-*}}`) are filled from the fresh 3-arm benchmark
before posting. Post as a single PR comment after the branch is force-pushed
to the reworked design.
...
mid-run: {{PLACEHOLDER-CHAOS-SUMMARY}}. The reworked design has no memory to
trust, so repair is just the normal level-triggered pass.
...
startup-latency observations — 328 observations for 320 claims in one burst
on main; (2) stale pre-adoption views re-entering adoption and issuing doomed
409 writes, which with a momentarily drained warm queue can duplicate a
cold-create POST for an already-bound claim; (3) an unlocked stale status
patch overwriting a newer bound status (a visible Ready flap). A/B numbers
from the fresh run: {{PLACEHOLDER-ROI}}.
...
trip, not a watch storm. Our watch probe confirms it:
{{PLACEHOLDER-WATCH-PROBE-CONFIRM}}. The measurable wins are in the
non-identical cases (the doomed adoption writes, the stale overwrites, and
the metric double-counts), and the new PR body scopes its claims accordingly.
