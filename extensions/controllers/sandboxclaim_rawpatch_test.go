...
// non-status metadata patches land on the claim (the observability annotation
// write is gone), the only remaining claim write is the adoption's optimistic
// -lock Update, and the adoption still completes with the claim status bound
// to the warm sandbox. A control run with the flags off shows the one
// annotation patch the flag removes.
func TestDisableFlagsWarmAdoptionClaimWrites(t *testing.T) {
	run := func(t *testing.T, disableFlags bool) (claimPatches, claimUpdates int, bound *extensionsv1beta1.SandboxClaim) {
		scheme := newScheme(t)
		...
		return claimPatches, claimUpdates, updatedClaim
	}

	t.Run("flags on: zero claim metadata patches", func(t *testing.T) {
		patches, updates, claim := run(t, true)
		if patches != 0 {
			t.Errorf("expected 0 non-status claim patches with the flags enabled, got %d", patches)
		}
	...
	t.Run("flags off control: one annotation patch", func(t *testing.T) {
		patches, updates, claim := run(t, false)
		if patches != 1 {
			t.Errorf("expected exactly 1 claim metadata patch (observability annotations) with flags off, got %d", patches)
		}
		if updates != 1 {
			t.Errorf("expected exactly 1 claim update (adoption optimistic lock), got %d", updates)
		}
