...
//
// Motivation (perf investigation): the DeepCopy+client.MergeFrom pattern
// serializes the ENTIRE object twice (base and modified) and diffs the two
// JSON documents just to emit a two-key annotation or label patch. On the
// claim-adoption hot path that full-object serialize/diff was measured at
// double-digit percent of controller CPU (persistStampedAnnotations alone was
// 15.8% in the round-2 profile). The helpers here marshal ONLY the intended
// keys, producing byte-identical wire payloads to what MergeFrom would have
// computed for the same metadata-only mutation, at O(patch) instead of
// O(object) cost.
//
