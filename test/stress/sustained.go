...
//
// Where claims-warm fires N claims simultaneously (a one-shot burst), this
// phase models steady-state arrivals: claims arrive as a Poisson process at
// --sustained-rate claims/s for --sustained-seconds, each claim is deleted
// --claim-dwell after it becomes Ready, and the warm pool controller refills
// continuously in the background. The question it answers is the one a burst
// cannot: do create→Ready latencies HOLD over time, or do they degrade as
// adoption, refill, and teardown churn compound? Degradation shows up
// directly in the rolling per-10s-window p50/p90/p99 (summary.json
// sustainedWindows, also printed in the report).
//
// Pool sizing (per namespace): replicas = ceil(rate/namespaces ×
...
// latency, so the steady-state inventory deficit is rate × refill-latency.
// The headroom must therefore cover the controller's WORST-CASE refill
// latency: --sandbox-warm-pool-replenish-delay (if set, e.g. 20s on the
// benchmark configs) + cold sandbox launch p99 (~4-6s on the benchmark
// clusters). The 10s default assumes no replenish delay; raise it to
// replenish-delay + ~10s otherwise, or the pool runs dry and claims
// cold-start (visible as window-latency cliffs — real degradation signal,
// but of the pool sizing, not the adoption path).
//
// Cluster capacity: peak concurrent pods ≈ pool total (kept full by refill)
// + rate × (ready-latency + dwell + deletion pipeline) adopted-but-not-yet-
...
// This exists for controller shard testing (SCALE-ROADMAP R4.4): namespaces
// are the shard key, so an 8-shard controller needs arrivals spread over 8
// namespaces before shards share the load. With N=1 the phase runs entirely
// in the main test namespace.
...
// Extension GVRs used by the sustained phase to build per-namespace clients.
var (
...
// ceil(rate/namespaces × headroom-seconds). See the package comment for the
// sizing rationale.
...
// sustainedExpectedClaims is the expected arrival count (rate × duration);
...
type sustainedNamespaceClients struct {
	name     string
	sandbox  dynamic.ResourceInterface
...
func (s *stressTest) sustainedClientsFor(namespace string) sustainedNamespaceClients {
	return sustainedNamespaceClients{
		name:     namespace,
		sandbox:  s.mutateClient.Resource(gvrSandboxes).Namespace(namespace),
...
func (s *stressTest) runClaimsWarmSustainedPhase(ctx context.Context, number PhaseNumber) error {
	...
	// overlap, which is the steady state this phase exists to measure.
...
// and — for multi-namespace runs — the extra namespaces themselves. Like
// cleanupClaimsWarm, failures are logged rather than returned, and the
// end-of-run namespace deletion is the backstop for the main namespace.
func (s *stressTest) cleanupSustained(ctx context.Context, number PhaseNumber, namespaces []sustainedNamespaceClients, poolName, templateName string) {
	...
	// namespace to disappear, mirroring cleanupClaimsWarm.
	s.waitSustainedGone(ctx, number, func() (int, error) {
