...
func TestBuildPhaseRunsWarmPool(t *testing.T) {
	valid := Config{
		Phases:                 []string{string(PhaseWarmPoolOvercreate), string(PhaseWarmPoolUnschedulable)},
		WPPools:                20,
		WPReplicas:             25,
	...
	runs, err := buildPhaseRuns(&stressTest{cfg: valid})
	if err != nil {
		t.Fatalf("buildPhaseRuns(valid) error: %v", err)
	}
	if len(runs) != 2 || runs[0].name != PhaseWarmPoolOvercreate || runs[1].name != PhaseWarmPoolUnschedulable {
	...
	for _, tc := range invalid {
		t.Run(tc.name, func(t *testing.T) {
			cfg := valid
			tc.mutate(&cfg)
			if _, err := buildPhaseRuns(&stressTest{cfg: cfg}); err == nil {
				t.Errorf("buildPhaseRuns accepted invalid config (%s)", tc.name)
			}
		})
	}
}
