...
//     (Sandbox status propagation). The pod spec the controller WRITES is
//     built from the Sandbox's PodTemplate (newPodForCR), never from the
//     cached pod, and every pod write in this repo is a metadata-only merge
//     patch diffed against the same transformed cache object — stripped
//     fields appear on neither side of the diff, so they can never leak into
//     (or be deleted by) a patch. See TestPodCacheTransformMergePatchUnaffected.
//
...
func (r *SandboxReconciler) reconcilePod(ctx context.Context, sandbox *sandboxv1beta1.Sandbox, nameHash string) (*corev1.Pod, error) {
	...
	ensurePodNameAnnotation := func(podName string) error {
		...
		patch, err := rawpatch.Annotations(map[string]string{sandboxv1beta1.SandboxPodNameAnnotation: podName})
		if err != nil {
