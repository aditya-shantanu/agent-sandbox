...
// RequeueAfter-based write deferral: the alternative mechanism to the
// internal/writebehind background flusher, per the review suggestion that a
// controller "can also return with RequeueAfter ... and get some of this for
// free". Instead of enqueueing a pending mutation into a flusher, the
// reconciler skips the recoverable write and asks the workqueue to redeliver
// the request once the coalescing window has elapsed; the redelivered pass
// recomputes the desired state from informer state and writes once. See
// DESIGN-NOTE.md at the repo root for the mechanical comparison.
...
//
// This timestamp map is the one piece of in-memory state this variant cannot
